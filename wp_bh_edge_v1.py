...
try:
    with open('nodes_73_v1.txt', 'r') as file:
...
def read_plc_nodes(plc, plc_name):
    ...
    parking_map = create_parking_map_from_file(plc, plc_name, current_date, current_time, "parking_maps73.txt", queue_data_list)
...
def send_data_continuously(interval, plc):
    ...
    while not stop_thread.is_set():
        ...
        # These send operations are NOT PLC operations, so they can be outside the lock
            t2 = time.time()
            for dataset in (data1, parking_map):
                if dataset:
...
# Ensure parking_maps73.txt exists with 73 entries
parking_map_nodes = [
    {"name": f".PLC_To_Server.Parking_Map.Token_No[{i}]", "type": "PLCTYPE_INT"}
    for i in range(1, 74)
]
with open("parking_maps73.txt", "w") as f:
    json.dump(parking_map_nodes, f, indent=4)
